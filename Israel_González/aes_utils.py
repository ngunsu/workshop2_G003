...
def desencriptar(texto_encriptado: str, llave: str) -> str:
    ...
    # IV (16 bytes) + al menos un bloque AES (16 bytes).
    if len(data) < AES.block_size * 2:
        raise ValueError("El texto encriptado es demasiado corto")

    # CBC requiere bloques completos para el ciphertext.
    if (len(data) - AES.block_size) % AES.block_size != 0:
        raise ValueError("Longitud de ciphertext invalida")
