...
def test_payload_demasiado_corto_falla():
    # Menos de IV+ciphertext minimo (32 bytes totales).
    with pytest.raises(Exception):
        desencriptar("00" * 16, "llave")


def test_payload_longitud_invalida_falla():
    # IV completo (16 bytes) + 1 byte de ciphertext: invalido para CBC.
    with pytest.raises(Exception):
        desencriptar("00" * 17, "llave")
