...
def encriptar(texto: str, llave: str) -> str:
    ...
    ct = cipher.encrypt(
        pad(texto.encode("utf-8"), AES.block_size)
    )  # le ponemos utf 8 envés de ascii para soportar caracteres especiales
    return (
        iv + ct
    ).hex()  # cambiamos ct.hex() por (iv + ct).hex() para incluir el IV en el resultado final
...
def desencriptar(texto_encriptado: str, llave: str) -> str:
    ...
    return unpad(cipher.decrypt(ct), AES.block_size).decode(
        "utf-8"
    )  # cambiamos unpad(cipher.decrypt(ct), AES.block_size * 2).decode() por unpad(cipher.decrypt(ct), AES.block_size).decode("utf-8") para corregir el tamaño del bloque y asegurar la decodificación UTF-8
