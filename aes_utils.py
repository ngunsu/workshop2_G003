...
def encriptar(texto: str, llave: str) -> str:
    ...
    ct = cipher.encrypt(pad(texto.encode("utf-8"), AES.block_size))  # Cambie el encode "ascii" por "utf-8" para soportar caracteres especiales
    return (
        iv + ct
    ).hex()  # Falta el iv antes de el ciphertext, entonces desencriptar() asume los primeros 16 bytes como iv y el resto como ciphertext
...
def desencriptar(texto_encriptado: str, llave: str) -> str:
    ...
    ).decode()  # Le saque el *2 de AES.block_size porque esta buscando el doble del pad que le llega entonces tiraba el error de padding invalido
