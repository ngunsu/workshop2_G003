...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        texto_encriptado = typer.prompt("Texto encriptado (hex)")
        llave = typer.prompt("Llave secreta")
        try:
            resultado = desencriptar(texto_encriptado, llave)
            console.print(
                Panel(f"[bold cyan]{resultado}[/]", title="Texto Desencriptado", expand=False)
            ...
            console.print("[bold red]Error:[/] llave incorrecta o texto encriptado invalido.")
            raise typer.Exit(code=1)
    else:
        app()
